...
static float yawt;
static bool r_cusd = false;
static bool p_cusd = false;

void sensfusion6GetEulerRPY(float* roll, float* pitch, float* yaw)
{
// Made changes to handle quaternion to euler angles
// when yaw hits singularities near pitch +/- 90 degrees
// and pitch angle is +/- 180 degrees instead of +/- 90 degrees
...
//pitch handler
  if ((gx < 0.1) && (gx > -0.1)) 
  {
    *pitch = asinf(gx) * 180 / M_PI_F;
    if (p_cusd)
    {
      if (gx>=0) *pitch = 180 - *pitch; else *pitch = -180 - *pitch;
    }
  }
  else
  {
    *pitch = atan2f(gx, gz) * 180 / M_PI_F;
  }
  if (gz>0)
  {
...
//roll handler
  if ((gy < 0.1) && (gy > -0.1)) 
  {
    *roll = asinf(gy) * 180 / M_PI_F;
    if (r_cusd)
    {
      if (gy>=0) *roll = 180 - *roll; else *roll = -180 - *roll;
    }    
  }
  else
  {
...
// yaw handler
  if ((gx > 0.9) || (*pitch >= 90))  //pitch near + 90 && upside down
  {
    yawt = 2 * atan2f(-q1, q2) * 180 / M_PI_F + 360;
    AdjAngle(&yawt);
  }
  else if ((gx < -0.9) || (*pitch <= -90))  //pitch near -90 && upside down 
  {
    yawt = 2 * atan2f(-q1, q2) * 180 / M_PI_F;
    AdjAngle(&yawt);    
  }
  else
  {
    yawt = atan2f(2*(q0*q3 + q1*q2), q0*q0 + q1*q1 - q2*q2 - q3*q3) * 180 / M_PI_F;
  }
//  temp = *yaw;
  *yaw = yawt;  
//  yawt = temp;
